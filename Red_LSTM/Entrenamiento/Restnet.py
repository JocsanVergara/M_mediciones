...
files_joined = os.path.join('Entre*.csv')
...
Dir = './best_only_ResNet/model-ep{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}.h5'
...
history_4 = Regression_Model.fit(X_train, train_y, epochs=200, batch_size=1024, verbose=1, validation_split=0.1, shuffle=True, callbacks=callbacks)  ##128
...
Predictions = Regression_Model.predict(test_x, verbose=1)
